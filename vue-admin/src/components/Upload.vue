<template>
  <div class="upload">
    <div v-if="limitNum == 1">
      ...
      <el-upload :ref="refName" action="" v-if="uploadStatus" :show-file-list="false" list-type="card"
        :auto-upload="false" :limit="limitNum" class="avatar-uploader picture-card" :on-change="
          (file) => {
            uploadChange(file);
          }
        " :on-exceed="
    ...
    <div v-else class="upload-box">
      <div v-for="item in imageArr" :key="item" class="img-box" :class="uploadStatus ? 'border-box' : ''">
        ...
        <div class="img-remove" v-if="uploadStatus">
          <el-icon @click="removeImageArr(item)">
            <Delete />
          </el-icon>
        </div>
      </div>
      ...
      <el-upload :class="canUpload ? '' : 'dis-no'" :ref="refName" action="" v-if="uploadStatus" :show-file-list="false"
        :auto-upload="false" :limit="limitNum" class="avatar-uploader" :on-change="
          (file) => {
            uploadChange(file);
          }
        ">
