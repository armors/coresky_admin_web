  ...
  <div class="config-group">
    ...
    <template v-else>
      ...
      <div style="text-align: right">
        <el-button v-permission="['*']" :loading="loading" type="danger" v-if="editKey !== 'edit'"
          @click="editConfig(dataKey2, 'edit')">{{ $t("global.edit") }}</el-button>
        <el-button v-permission="['*']" :loading="loading" type="primary" v-if="editKey === 'edit'"
          @click="updateConfig(dataKey2)">{{ $t("global.save") }}</el-button>
        <el-button v-permission="['*']" :loading="loading" v-if="editKey === 'edit'" @click="cancelConfig(1)">
      ...
      <template v-if="isPreset">
        ...
        <deploy-contract :label="$t('config.Multicall')" propKey="multicall" :dataKey="dataKey" :editKey="editKey"
          :showData="showData" @setEditKey="editKey = $event" :deployFn="deploy" />
        <deploy-contract :label="$t('config.NFT721')" propKey="nft721" :dataKey="dataKey" :editKey="editKey"
          :showData="showData" @setEditKey="editKey = $event" :deployFn="deploy" />
        <deploy-contract :label="$t('config.TransferProxy')" propKey="transferProxy" :dataKey="dataKey"
          :editKey="editKey" :showData="showData" @setEditKey="editKey = $event" :deployFn="deploy" />
        <deploy-contract :label="$t('config.TransferProxyForDeprecated')" propKey="transferProxyForDeprecated"
          :dataKey="dataKey" :editKey="editKey" :showData="showData" @setEditKey="editKey = $event"
          :deployFn="deploy" />
        <deploy-contract :label="$t('config.ERC20TransferProxy')" propKey="erc20TransferProxy" :dataKey="dataKey"
          :editKey="editKey" :showData="showData" @setEditKey="editKey = $event" :deployFn="deploy" />
        <deploy-contract :label="$t('config.ExchangeState')" propKey="exchangeState" :dataKey="dataKey"
          :editKey="editKey" :showData="showData" @setEditKey="editKey = $event" :deployFn="deploy" />
        <deploy-contract :label="$t('config.ExchangeOrdersHolder')" propKey="exchangeOrdersHolder" :dataKey="dataKey"
          :editKey="editKey" :showData="showData" @setEditKey="editKey = $event" :deployFn="deploy" />
        <deploy-contract :label="$t('config.NftExchange')" propKey="nftExchange" :dataKey="dataKey" :editKey="editKey"
          :showData="showData" @setEditKey="editKey = $event" :deployFn="deploy" />
        ...
        <div class="config-item" v-for="(v, k) in operatorMap" :key="k">
          <div class="item-value" v-for="(v1, k1) in v" :key="k1">
            <el-button :loading="loadingMap[`${k}_${k1}`]" :type="v1 ? 'success' : 'warning'" :disabled="v1"
              v-permission="['POST /admin/config/update']" plain @click="_addOperator(k, k1)">
...
import { configUpdate } from '@/api/common'
