  ...
  <div class="app-container">
    ...
    <el-form ref="queryForm" label-width="100px" size="small" :inline="true">
      <el-form-item label="" class="wItem">
        <el-input v-model="listQuery.username" clearable :placeholder="$t('systemManagement.placeholder1')"
          style="width:200px">
        </el-input>
      </el-form-item>
      <el-form-item>
        ...
        <el-button v-permission="['POST /admin/admin/create']" type="primary" size="mini" @click="handleCreate">
    ...
    <el-table v-loading="listLoading" :data="list" :element-loading-text="$t('util.loadingText')" border
      ...
      <el-table-column align="center" :label="$t('systemManagement.administratorID')" prop="id" />
      ...
      <el-table-column align="center" :label="$t('systemManagement.administratorAvatar')" prop="avatar">
        <template #default="scope">
          <media v-if="scope.row.avatar" :url="STATIC_URL+scope.row.avatar" type="image" :isPreview="true"></media>
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('systemManagement.administratorRoles')" prop="roleIds">
        <template #default="scope">
          <el-tag v-for="roleId in scope.row.roleIds" :key="roleId" type="primary" class="mr-20px">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px"
        ...
        <el-form-item :label="$t('systemManagement.administratorAvatar')" prop="avatar">
          <el-upload ref="upload" action="" :auto-upload="false" :show-file-list="false" :file-list="filelist"
            :limit="1" class="avatar-uploader" accept="image/jpg, image/jpeg, image/png, image/gif" :on-change="
              (file) => {
...
import { adminList, roleOptions, adminUpdate, adminCreate, adminDelete, storageCreate } from '@/api/common'
export default {
  name: "Admin",
  components: {},
  mixins: [base],
